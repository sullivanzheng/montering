...
//Calculate integer part of writhe and AlexPoly(-1). Numerically stable since drx is not used 
//to test intersection. A more stable method is used.
long CircularChain::_kndwr_topl_update_stable(double &topl,long &ierr){
	const long isi=801;
    ...
    static long jwr;
    static double zet;
    static double pdx1, pdy1, pdx2, pdy2, wx, wy;
    static long jmin;
    static double pdx12;
...
/* ic2(i) - number of overgoing segment for i-th intersection */
    ...
    for (n1 = 1; n1 <= i__1; ++n1) {
		...
		for (n2 = jr4; n2 <= i__2; ++n2) {
			...
			wx = px2 - px1;
			if ( wx > cutoff || wx < -cutoff) continue;
			else{
				wy = py2 - py1;
				if ( wy > cutoff || wy < -cutoff) continue;
			}
			...
			if (d__ < deps && d__ > -deps) 	continue; //d=0 when parallel.

			//Numerically stable way to calculate intersection point.
			//Reference: http://softsurfer.com/Archive/algorithm_0104/algorithm_0104B.htm
			double s1, s2;
			s1 = (-pdy2*wx+pdx2*wy)/d__;
			if (s1<0 || s1>=1) continue;
			s2 = (-pdy1*wx+pdx1*wy)/d__;
			if (s2<0 || s2>=1) continue;

			if ((double) n4 > range1) goto L1002;
		    
			rz1 = s1 * dz[n1 - 1] + z[n1 - 1];
			rz2 = s2 * dz[n2 - 1] + z[n2 - 1];

			rx = s1 * pdx1 + px1;
			cx[n4 - 1] = rx; //stored for later renumeration.
...
/* ic2(i) - number of overgoing segment for i-th intersection */
	...
	for (n2 = jr4; n2 <= i__2; ++n2) {
	    ...
	    drx = (py2 * pdx12 - px2 * pdx1 * pdy2 - py1 * pdx12 + px1 * pdx2 
		    * pdy1) / d__; // x coordinate of intersection.
	    if (px1 <= px11) {

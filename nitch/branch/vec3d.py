...
def makeCircle(n,seglen=1.0):
    dx=[seglen*cos(2*pi/n*i) for i in range(n)]
    dy=[seglen*sin(2*pi/n*i) for i in range(n)]
    dz=[0]*n
    return dx,dy,dz
...
def make_shunapsised_heterogeneous_circular(core):
    circlesize=96
    InsBreak=11
    dx1,dy1,dz1=makeCircle(circlesize,3.0)
    dx2,dy2,dz2=readJmol(core,1)
##  write_Jmol_file_dx(core+"ini.txt",dx2,dy2,dz2)

    dx2,dy2,dz2= \
    ConnectUsingBreakCircle(dx1,dy1,dz1,
                        dx2,dy2,dz2,1,circlesize*2/3,InsBreak)
##  ConnectUsingBreakCircle(dx1,dy1,dz1,
##                            dx2,dy2,dz2,h,m,InsBreak)
##  write_Jmol_file_dx(core+"intermediate.txt",dx2,dy2,dz2)
    dx2,dy2,dz2=dx2[1:],dy2[1:],dz2[1:]
    dx2,dy2,dz2= \
    integrateToCircular(dx2,dy2,dz2,
                         dx1,dy1,dz1,int(circlesize/3*2),circlesize*5/6)
    write_Jmol_file_dx(core+"main.txt",dx2,dy2,dz2)    

if __name__=='__main__':
    makeCircle_File("nitch.txt",18+22)
